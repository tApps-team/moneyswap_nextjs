import { Metadata } from "next";
import { PaymentServicesPage } from "@/views/payment-services";
import { getAllPaymentServices, getPaymentServicePage, stripHtmlToText } from "@/entities/strapi";
import { routes } from "@/shared/router";
import { Breadcrumbs } from "@/shared/ui";

const baseUrl = process.env.NEXT_PUBLIC_SITE_BASE_URL || "";

const DEFAULT_DESCRIPTION =
  "Рейтинг сервисов для оплаты зарубежных подписок, игр и покупок из России — комиссии, способы оплаты и отзывы на MoneySwap.";

function getPageDescription(headerContent: { paragraph?: { content?: string } }[] | undefined) {
  const paragraph = headerContent?.find(
    (item) => item && "paragraph" in item && item.paragraph?.content,
  );
  if (!paragraph?.paragraph?.content) {
    return DEFAULT_DESCRIPTION;
  }
  return stripHtmlToText(paragraph.paragraph.content).slice(0, 160);
}

export async function generateMetadata(): Promise<Metadata> {
  const { data: page } = await getPaymentServicePage();
  const title = page?.title
    ? `${page.title} | MoneySwap`
    : "Оплата зарубежных сервисов | MoneySwap";
  const description = getPageDescription(page?.header_content);

  return {
    title,
    description,
    metadataBase: new URL(baseUrl),
    openGraph: {
      title,
      description,
      url: `${baseUrl}${routes.payment_services}`,
      siteName: "MoneySwap",
      images: [{ url: "/og_logo.svg", width: 400, height: 283, alt: "MoneySwap" }],
      locale: "ru-RU",
      type: "website",
    },
    alternates: {
      canonical: `${baseUrl}${routes.payment_services}`,
    },
  };
}

export default async function Page() {
  const services = await getAllPaymentServices();

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "Рейтинг сервисов оплаты зарубежных сервисов",
    description: DEFAULT_DESCRIPTION,
    itemListElement: services.map((service, idx) => ({
      "@type": "ListItem",
      position: idx + 1,
      name: service.name,
      url: `${baseUrl}${routes.payment_services}/${service.slug}`,
    })),
  };

  return (
    <>
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(jsonLd).replace(/</g, "\\u003c"),
        }}
      />
      <Breadcrumbs />
      <PaymentServicesPage />
    </>
  );
}
