...
import { VcMarketType, VirtualCard } from "@/entities/strapi";
import { cn } from "@/shared/lib";
import { routes } from "@/shared/router";

interface VcCatalogProps {
  cards: VirtualCard[];
...
const TABS: { value: VcMarketType; label: string }[] = [
  { value: "international", label: "Международные" },
  { value: "russian", label: "Российские" },
];

export const VcCatalog: FC<VcCatalogProps> = ({ cards, marketType, page, totalPages }) => {
  return (
    <div className="grid gap-6">
      ...
      <div className="flex gap-2 p-1.5 rounded-[12px] bg-new-dark-grey w-full lg:w-fit">
        {TABS.map((tab) => (
          <Link
            key={tab.value}
            href={`${routes.virtual_cards}?market=${tab.value}&page=1`}
            scroll={false}
            className={cn(
      ...
      */}
      ...
      {totalPages > 1 ? (
        <div className="flex justify-center">
          <Pagination currentPage={page} totalPages={totalPages} route={routes.virtual_cards} />
        </div>
      ) : null}
    </div>
  );
