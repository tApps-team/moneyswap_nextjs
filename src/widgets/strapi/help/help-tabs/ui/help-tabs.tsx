...
export const HelpTabs: FC<HelpTabsProps> = ({ isFaq, setIsFaq, faqTab, setFaqTab }) => {
  return (
    <section
      className={`grid grid-flow-row ${isFaq ? "lg:gap-y-0 md:gap-y-[50px] mobile-xl:gap-y-8 gap-y-6" : ""}`}
    >
      <div className="max-w-[80%] grid grid-cols-2 gap-6">
        <div
      ...
      {isFaq && (
        <div className="lg:mt-[60px] flex flex-row mobile:gap-10 gap-6">
          <div
            className={`cursor-pointer mobile-xl:text-lg mobile:text-sm text-2xs font-normal pb-2 ${faqTab === faqTypeTabs.for_user ? "text-yellow-main border-b-yellow-main border-b-[2px]" : "text-font-light-grey"}`}
            onClick={() => setFaqTab(faqTypeTabs.for_user)}
          >
            {faqTypeTabs.for_user}
          </div>
          <div
            className={`cursor-pointer mobile-xl:text-lg mobile:text-sm text-2xs font-normal pb-2 ${faqTab === faqTypeTabs.for_partner ? "text-yellow-main border-b-yellow-main border-b-[2px]" : "text-font-light-grey"}`}
