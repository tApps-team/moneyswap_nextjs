...
export const AccordionList: FC<MainFaqs> = ({ data }) => {
  return (
    <Accordion type="single" collapsible className="w-full grid gap-4 lg:gap-6">
      {data?.map((faq) => (
        <AccordionItem
          value={`Value-${faq?.id}`}
          key={faq?.id}
          className="grid grid-flow-row rounded-[15px] bg-new-grey md:py-6 md:pl-8 md:pr-12 py-4 px-6"
        >
          <AccordionTrigger className="md:text-base mobile:text-sm text-[11px] font-normal [&[data-state=open]]:text-yellow-main tracking-wider text-start color-[#fff] p-0 border-b-0 md:[&>svg]:-mr-7 mobile-xl:[&[data-state=open]]:text-white [&[data-state=open]>svg]:stroke-yellow-main hover:text-white mobile-xl:leading-6 leading-4 [&>svg]:w-5 [&>svg]:h-5 [&>svg]:stroke-[2px] [&>svg]:border-[1px] [&>svg]:border-white [&>svg]:rounded-[5px]">
            {faq?.question}
          </AccordionTrigger>
